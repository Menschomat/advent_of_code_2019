...
def hacky_solution(in_dat):
    ...
    print(sum([int_list.append(floor(int(x) / 3) - 2) or floor(int(x) / 3) - 2 for x in int_list if
               (floor(int(x) / 3) - 2) > 0]))
...
def more_convenient_solution(in_dat):
    ...
    int_list = [int(val) for val in in_dat.read().split('\n')]
    print(sum([append_with_return(floor(int(x) / 3) - 2, int_list) for x in int_list]))
